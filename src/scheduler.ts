...
class SchedulerElement extends CustomElement {
  ...
  connectedCallback() {
    Array.from(this.shadowRoot!.querySelectorAll("[control]")).forEach((elem) =>
  ...
  public attributeChangedCallback(
    ...
    if (attr === "timecode") {
      ...
      this.setSchedule(timecode);
    }
  }
  onControlChange(ev: Event) {
    const controlName = ev.target!.parentNode.getAttribute("control");
    const value = ev.target.value;
    const storageKey = this.getAttribute("control") as string;
    if (localStorage.getItem("DJ-APP-CONFIG")) {
      const config = JSON.parse(
        localStorage.getItem("DJ-APP-CONFIG") as string
      );
      const timecode = {
        ...config[storageKey],
        [controlName]: value,
      ...
      localStorage.setItem("DJ-APP-CONFIG", JSON.stringify(config));
    }
  }
  setSchedule(timecode: TimeCode) {
